...
public class VideoCapture extends Activity implements SurfaceHolder.Callback {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		currentTime = (TextView) findViewById(
				resources.getIdentifier("currenttime", "id", getApplication().getPackageName()));
		durationTime = (TextView) findViewById(
				resources.getIdentifier("duration", "id", getApplication().getPackageName()));
	...
	private void prepareRecorder() {
		...
		if (camcorderProfile.fileFormat == MediaRecorder.OutputFormat.THREE_GPP) {
			try {
				File newFile = File.createTempFile("AutoRecordVideo", ".3gp",
			...
			try {
				File newFile = File.createTempFile("AutoRecordVideo", ".mp4",
						Environment.getExternalStorageDirectory());
				recorder.setOutputFile(newFile.getAbsolutePath());
				outFile = newFile;
			} catch (IOException e) {
